...
%   t       [s]      time since Oxz plane crossing
%
% OUTPUTS
%   r0      [km]     position in ECI frame
%   v0      [km/s]   velocity in ECI frame

omega_earth = 72.921e-6; % [rad/s] Earth rotation speed
n_Earth = [0; 0; omega_earth];  % [rad/s] rotation vector

lambda = omega_earth * t;  % [rad] longitud S1 w/ respect t0 S0 

% Rotation about Z axis of ECI (longitude)
R_z = [cos(lambda)  -sin(lambda)   0;
       sin(lambda)   cos(lambda)   0;
       0             0             1];
% Rotation about Y axis of ECI (latitude)
R_y = [sin(phi)   0    cos(phi); %
       0          1    0;
       -cos(phi)  0    sin(phi)];

% Total rotation matrix from SEZ to ECI ref.framces
R_tot = R_z * R_y;
